...
| `Features/` | REPR + messaging use cases by BC — Endpoint, Request, Response, Validator, Summary, Command or Query, Handler |
| `Cqrs/` | CQRS markers: `IQuery` / `IQueryHandler` (extend FE `ICommand` / `ICommandHandler`) |
| `Infrastructure/` | `Settings/` (`sealed record` POCOs with `init`, e.g. `DatabaseSettings`), providers (`IUserProvider`, `IHashProvider`), `Persistence/` (`AppDbContext`, EF configs by BC) |

`AddInfrastructure` registers Settings, providers, and DbContext. Command handlers are discovered with FastEndpoints. One public type per file.
